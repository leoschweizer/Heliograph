...
- (void)testNumberOfArguments {
	...
	HGMethodMirror *method1 = [class methodWithSelector:@selector(UTF8String)];
	XCTAssertNotNil(method1);
	XCTAssertEqual([method1 numberOfArguments], 0);
	HGMethodMirror *method2 = [class methodWithSelector:@selector(initWithFormat:locale:)];
...
- (void)testReturnType {
	...
	HGMethodMirror *method = [class methodWithSelector:@selector(deleteCharactersInRange:)];
...
- (void)testArgumentTypes {
	HGClassMirror *class = reflect([NSString class]);
	HGMethodMirror *method = [class methodWithSelector:@selector(stringByFoldingWithOptions:locale:)];
